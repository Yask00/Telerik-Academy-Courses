...
let ShoppingCart = require('./cart').ShoppingCart;

describe('Task Gosho', function() { // GROUP
	...
	it('Equal test', function() { // TEST
		const array = [1, 2, 3];
		expect(array).to.equal(array);
	});

	it('Eql test', function() { // TEST
		const array1 = [1, 2, 3];
		const array2 = [1, 2, 3];
		expect(array1).to.eql(array2);
	});

	describe('Zero tests', function() { // GROUP
